...
# Classes - Consider having a Game class
class Board(object):
	...
	def drawBoard(self):			# TODO: Add a parameter to flip the board and show it from black's perspecctive
		if self.gameMode == 2:
		...
		if self.whiteView:
			...
			for x in range(8):
				for y in range(8):
					...
					if (hasattr(self.boardData.get((x,y)), 'color')):
						if self.boardData.get((x,y)).color == 'White':
							win.blit(whiteSprites[self.boardData[(x,y)].pieceCode], (60 * (8-y), 60 * (x+1)))
							print(self.boardData[(x,y)].pieceCode, (60 * (8-y), 60 * (x+1)))
						else:
							win.blit(blackSprites[self.boardData[(x,y)].pieceCode], (60 * (8-y), 60 * (x+1)))
...
# Game loop
board.drawBoard()		# TODO: Add a conditional to flip the board according to whose turn it is, and allow drawBoard to take in another parameter
		...
		if event.type == pygame.MOUSEBUTTONDOWN:
			...
			if not board.gameOver:
				...
				if board.checkPendingPromotion():
					print('There is a pending promotion!')
					board.doPromotion()
					board.whiteTurn = not board.whiteTurn
					if board.kingInCheck():
						print('Check!')
